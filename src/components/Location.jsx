    ...
    <div className="main">
      <div className="main-container">
        ...
        <div
          ...
          style={{ fontSize: "14px", fontWeight: 600, color: "#888888" }}
        >
          동(읍,면) 이름으로 검색
        </div>
        ...
        <ul>
          {isSelected === false &&
            dummy
              .filter((e) => !!inputValue === true && e.includes(inputValue))
              .map((e, i) => (
                <li
                  key={i}
                  onClick={() => {
                ...
                >
                  <span className="result-items">{e}</span>
                </li>
              ))}
        </ul>
        <div
          className={`confirm-btn ${isSelected ? "active" : ""}`}
          {...(isSelected && {
            onClick: () => {
              navigate("/");
              dispatch({ type: "addLocation", payload: selectedValue });
            },
          })}
        >
          확인
