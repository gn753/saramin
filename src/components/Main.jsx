...
export default function Main() {
  ...
  const isLocationExist =
    state.locationReducer.length > 0 && state.locationReducer[0] !== "";
  console.log(state.locationReducer);
  const isCategorySelected = state.middleCategoryReducer.length > 0;
  const title =
    state.middleCategoryReducer.length > 1
      ? `${state.middleCategoryReducer[0]} 외 ${
          state.middleCategoryReducer.length - 1
        }개`
    ...
    <div className="main">
      <div className="main-container">
        ...
        <div className="init-container">
          {(isLocationExist || isCategorySelected) && (
            <div
              className="init"
              onClick={() => {
          ...
          {(isLocationExist || isCategorySelected) && (
            <img className="undo" src="prime_undo.png" alt="" />
          )}
        </div>

        <div
          className={`confirm-btn ${
            isLocationExist && isCategorySelected ? "active" : ""
          }`}
          onClick={() => {
            navigate("/maps");
          }}
